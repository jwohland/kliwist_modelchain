...
def make_s10_heatmaps():
    ...
    for experiment_id in ["rcp26", "rcp45", "rcp85"]:
        ...
        for experiment_family in ["CORDEX", "CMIP5"]:
            diff_agg = xr.open_dataset(
                "../output/country_aggregates/country_"
    ...
    for i, experiment_family in enumerate(["CORDEX", "CMIP5"]):
        ...
        sns.heatmap(
            ...
            vmin=-0.3,
            vmax=0.3,
            cbar_kws={"orientation": "horizontal", "label": "Ensemble mean wind speed change [m/s]"},
            cbar_ax=cbar_ax,
        )
        axs[i].set(xlabel="", ylabel="", title=experiment_family)

    plt.subplots_adjust(left=.09, right=.98, top=.95, wspace=0.2)
    plt.savefig(
        "../plots/countries/heatmap_mean_countries.jpeg",
        dpi=300,
    )
